...
function CreateEvent() {
	...
	const [eventTitle, setEventTitle] = React.useState("Titel der Veranstaltung");
	const [eventDesc, setEventDesc] = React.useState("Lorem ipsum dolor sit amet, consetetur sadipscing elitr...");
	...
	const handleNewPoll = () => {
		if (pollTitle != null) {
			...
			fetch(process.env.REACT_APP_API_URL + `/events/${params.eventId}/polls`, requestOptions)
	...
	const start = (poll) => {
		...
		fetch(process.env.REACT_APP_API_URL + `/polls/${poll._id}/start`, requestOptions)
			.then(response => getPolls())
			.catch(error =>
				console.log(error)
			)
		...
		return (
			<Stack spacing={1} direction="column">
				{polls.map((poll) => {
					...
					return (
						<Stack direction="row" key={poll._id} bgcolor={color} width="100%" sx={{ alignItems: 'center', justifyContent: 'space-between', paddingTop: '5px', paddingBottom: '5px' }}>
							<Typography sx={{ marginLeft: '5px' }}>{`${poll.title}`}</Typography>
							<Box sx={{ display: 'flex', flexDirection: 'row' }}>
								{!poll.activeUntil ?
									<Box>
										<Button onClick={() => start(poll)} size="large" variant="text" color="success" startIcon={<PlayArrowIcon />}>
											Starten
										</Button>
										<Button onClick={() => edit(poll)} size="large" variant="text" startIcon={<EditIcon />}>
											Bearbeiten
										</Button>
									</Box>
									:
									<Button onClick={() => results(poll._id)} size="large" variant="text" startIcon={<BarChartIcon />}>
										Ergebnisse
									</Button>
								}

								<Button size="large" variant="text" color="error" onClick={() => deletePoll(poll._id)} startIcon={<DeleteForeverIcon />}>
									Löschen
								</Button>
							</Box>
						</Stack>
	...
	return (
		<Box sx={{ width: windowWidth, margin: 'auto', marginTop: '25px' }}>
			...
			>
				...
				<DialogContent>
					<Stack direction="column" spacing={2}>
						<Typography variant="caption">Wenn Sie die Veranstaltung löschen, werden alle damit zusammenhängenden Daten unwiderruflich gelöscht.</Typography>
						<Button size="large" onClick={handleDeleteEvent} variant="contained" color="inherit" autoFocus>Veranstaltung löschen</Button>
						<Button size="large" onClick={handleDeleteEventDialogCloseDialog} variant="contained">Abbrechen</Button>
					</Stack>
				</DialogContent>
			</Dialog>
			<Dialog
			...
			>
				...
				<DialogContent>
					<Stack direction="column" spacing={2}>
						<TextField onClick={isMobile ? handleFocus : null} sx={{ width: '100%' }} id="standard-basic" label="Titel der Abstimmung" variant="standard" onChange={(event) => {
							setPollTitle(event.target.value);
						}} />
						<Button size="large" onClick={handleNewPoll} variant="contained" autoFocus>Abstimmung Erstellen</Button>
						<Button size="large" onClick={handleNewPollDialogCloseDialog} color="inherit" variant="contained">Abbrechen</Button>
					</Stack>
				</DialogContent>
			</Dialog>
			<Stack margin="15px" spacing={2}>
				<Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
					<Typography sx={{ typography: (isMobile ? 'h5' : 'h4') }}>{eventTitle}</Typography>
					<Button onClick={editEvent}><ModeEditIcon /></Button>
				</Box>
				<Typography sx={{ fontWeight: 'bold' }}>Abstimmungen</Typography>
				<Stack direction="column">
					{polls ?
						<Polls /> : null
					}
				</Stack>
				<ButtonShort size="large" variant="contained" color="success" onClick={handleCreateNewPoll}>
					+ Neue Abstimmung
				</ButtonShort>
				...
				<Typography sx={{ fontWeight: 'bold' }}>Veranstaltung löschen</Typography>
				<ButtonShort size="large" onClick={deleteEvent} variant="contained" color="error">
					Veranstaltung löschen
				</ButtonShort>
			</Stack>
			<Snackbar
