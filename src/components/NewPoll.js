...
function NewPoll() {
	...
	const addAnswer = () => {
		...
		fetch(
			process.env.REACT_APP_API_URL + `/polls/${pollId}/answers`,
			requestOptions
		)
			.then((response) => response.json())
			.then((result) => {
	...
	const deleteAnswer = (answer) => {
		...
		fetch(
			process.env.REACT_APP_API_URL + `/answers/${answer._id}`,
			requestOptions
		)
	...
	const getAnswers = () => {
		...
		fetch(
			process.env.REACT_APP_API_URL + `/polls/${params.pollId}/answers`,
			requestOptions
		)
			.then((response) => response.json())
			.then((data) => {
	...
	const save = () => {
		const requestOptions = {
			...
			body: JSON.stringify({
				poll_id: pollId,
				title: pollTitle,
				duration: voteTime / 60,
				votesPerParticipant: maxVotes,
				allowCustomAnswers: freeText,
			}),
		};
		fetch(
			process.env.REACT_APP_API_URL + `/polls/${params.pollId}`,
	...
	const start = () => {
		const requestOptions = {
			...
			body: JSON.stringify({
				...
				votesPerParticipant: maxVotes,
				allowCustomAnswers: freeText,
				allowMultipleVotesPerAnswer: multipleVotes,
			}),
		};
		fetch(
			process.env.REACT_APP_API_URL + `/polls/${params.pollId}`,
	...
	const startSaved = () => {
		...
		fetch(
			process.env.REACT_APP_API_URL + `/polls/${params.pollId}/start`,
			requestOptions
		)
			.then((response) => setRedirect(`/o/event/${params.eventId}`))
			.catch((error) => console.log(error));
		...
		<Box sx={{ width: windowWidth, margin: "auto" }}>
			...
			<FormGroup sx={{ marginTop: "10px" }}>
				<FormControlLabel
					control={
						<Checkbox
							onChange={checkFreeText}
							defaultChecked={false}
						/>
					}
					label="Freitextantworten zulassen"
				/>
				<FormControlLabel
					control={
						<Checkbox
							onChange={checkMultipleVotes}
							defaultChecked={true}
						/>
					}
					label="Mehrfachstimmen zulassen"
				/>
			</FormGroup>
			<Box
				sx={{
			...
			>
				<Typography sx={{ marginTop: "15px", fontWeight: "bold" }}>
					Anzahl der Stimmen pro Teilnehmer:
				</Typography>
				<TextField
