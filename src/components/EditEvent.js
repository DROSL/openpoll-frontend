...
function EditEvent() {
	...
	const saveAllChanges = () => {
		...
		const requestOptions = {
			...
			body: JSON.stringify({ title: eventTitle, description: eventDesc, open: true })
		}
	...
	const ButtonShort = React.forwardRef(function Alert(props, ref) {
		return <Button sx={{ width: (isMobile ? '100%' : '35%'), marginTop: '15px' }} {...props} />;
	});
	...
	return (
		<Box sx={{ width: windowWidth, margin: 'auto' }}>
			...
			<input id='selectFile' hidden type="file" name="file" onChange={changeHandler} />
			<input id='selectImage' hidden type="file" name="file" onChange={changeImageHandler} />
			<TextField multiline onClick={isMobile ? handleFocus : null} value={eventDesc} sx={{ width: '100%' }} id="standard-basic" label="Beschreibung" variant="standard" onChange={(event) => {
				setEventDesc(event.target.value); setChanges(true);
			}} />
			<Typography sx={{ marginTop: '15px', fontWeight: 'bold' }}>Datei</Typography>
			{isFilePicked ?
				(
					<Box>
						<Button size="large" sx={{ alignSelf: 'flex-start', }} color="inherit">
							<Stack sx={{ alignItems: 'center' }} direction="row" spacing={1}>
								<AttachFileIcon />
								<Stack direction="column">
									<Typography>{selectedFile.name}</Typography>
									<Typography variant="caption">{`${Math.trunc(selectedFile.size / 1024 / 1024)}MB`}</Typography>
								</Stack>
							</Stack>
						</Button>
				...
				:
				(<ButtonShort onClick={() => document.getElementById("selectFile").click()} size="large" variant="contained">
					+ Datei anhängen
				</ButtonShort>
				)
			}
			<Typography sx={{ marginTop: '15px', fontWeight: 'bold' }}>Branding</Typography>
			{isImagePicked ?
				(
					<Box>
						<Button size="large" sx={{ alignSelf: 'flex-start', }} color="inherit">
							<Stack sx={{ alignItems: 'center' }} direction="row" spacing={1}>
								...
								<Stack direction="column">
									...
									<Typography variant="caption">{`${Math.trunc(selectedImage.size / 1024 / 1024)}MB`}</Typography>
								</Stack>
							</Stack>
						</Button>
				...
				(
					<Stack spacing={2} direction="row" sx={{ alignItems: 'center' }}>
						<ButtonShort onClick={() => document.getElementById("selectImage").click()} size="large" variant="contained">
							+ Foto auswählen
						</ButtonShort>
			...
			{changes ? <ButtonShort onClick={saveAllChanges} size="large" variant="contained">
