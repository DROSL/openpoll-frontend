...
function ResultsP(props) {
	...
	const Results = () => {
		...
		return (
			<Stack spacing={1} direction="column">
				{results.map((result) => {
					var percentage = totalVotes > 0 ? (result.votes / totalVotes) * 100 : 0
					return (
						<Box key={result._id} id={result._id}>
							<Box sx={{ borderTopRightRadius: 10, borderBottomRightRadius: 10 }} color="white" bgcolor="primary.main" variant="contained" padding="13px" width={`${percentage}%`} />
							<Typography>{`${result.title} ${type === "percentage" ? Number.parseFloat(percentage).toFixed(2) + "%" : result.votes}`}</Typography>
						</Box>
					)
				})
				}
	...
	const getResults = () => {
		...
		fetch(process.env.REACT_APP_API_URL + `/polls/${params.pollId}/votes`, requestOptions)
			...
			<Box sx={{ width: windowWidth, margin: 'auto' }}>
				...
				<Box direction="horizontal">
					...
					<Stack sx={{ marginTop: '10px' }} direction="row">
						<Button size="large" onClick={handlePercentage} variant="text">
							Prozent
						</Button>
						<Button size="large" onClick={handleAbsolute} variant="text">
							Absolut
						</Button>
					</Stack>
				</Box>
