...
function EventViewP(props) {
	...
	const [eventDesc, setEventDesc] = React.useState("Beschreibung der Veranstaltung");
	...
	const handleVote = () => {
		setOpen(false);
		if (pollId != null) setRedirect(`/p/event/${params.eventId}/poll/${pollId}`); else {
	...
	return (
		<React.Fragment>
			<Box sx={{ width: windowWidth, margin: 'auto', marginTop: '25px', flexDirection: 'column', display: 'flex', alignItems: 'center' }}>
				<Box>
							...
							<Button size="large" sx={{ alignSelf: 'flex-start', }} color="inherit">
								<Stack sx={{ alignItems: 'center' }} direction="row" spacing={1}>
									...
									<Stack direction="column">
										...
										<Typography variant="caption">2 MB</Typography>
									</Stack>
					...
					{pollActive ?
						<Stack spacing={2} direction="column" sx={{
							...
							<Button size="large" sx={{ width: '80%', marginTop: '10px' }} onClick={handleVote} variant="contained">An Abstimmung teilnehmen</Button>
						</Stack> : null}
				</Box>
				<Box
					sx={{
				...
				>
					...
					<DialogContent>
						<Stack direction="column" spacing={2}>
							...
							<Button size="large" onClick={handleVote} variant="contained" autoFocus>Teilnehmen</Button>
							<Button size="large" onClick={handleClose} variant="contained" color="inherit">Ignorieren</Button>
