...
function VoteP(props) {
	...
	const handleResults = () => {
		if (votes > 0) {
			...
			setRedirect(
				`/p/event/${params.eventId}/poll/${params.pollId}/results`
			);
		}
	};
		...
		<React.Fragment>
			...
			>
					...
					<DialogContent>
						<Stack direction="column" spacing={2}>
							<Typography variant="caption">
								Sie haben noch nicht alle Ihre Stimmen
								verwendet. Wenn Sie fortfahren, verfallen Ihre
								restlichen Stimmen.
							</Typography>
							<Button
								size="large"
								onClick={handleProceedResults}
								variant="contained"
								autoFocus
							>
								Zu den Ergebnissen
				...
				>
					...
					<Typography variant="caption">{votes} Stimme(n)</Typography>
				</Box>
